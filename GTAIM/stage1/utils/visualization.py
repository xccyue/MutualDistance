...
def render_motion_point_in_scene(
    smplx_folder: str,
    save_folder: str,
    scene_mesh: trimesh.Trimesh=None,
    pkl: tuple=None, 
    ...
    S =trimesh.Scene()
    select_vertex = vertices[:,marker,:]
    for p in select_vertex:
        label_mesh = trimesh.creation.box(extents=[0.1, 0.1, 0.1]) # Replace with the desired size and shape of the labeled point
        label_mesh.apply_translation(p)
        label_mesh.visual.face_colors = [0,0,0.5]
        label_mesh.visual.face_colors = [label_mesh.visual.face_colors[0]] * len(label_mesh.faces)
        S.add_geometry(label_mesh)
    
    # p = trimesh.points.PointCloud(select_vertex)
    # S.add_geometry(p)
    ...
    S.export(os.path.join(save_folder, 'point_{}.ply'.format(marker)))
...
def render_reconstructed_motion_in_scene(
    ...
    if pkl_his is not None:
        body_vertices_his, body_faces_his, _ = SMPLX_Util.get_body_vertices_sequence(smplx_folder, pkl_his, num_betas=num_betas)
    else:
        pkl_his = None
    ## rendering
    ...
    material_his = pyrender.MetallicRoughnessMaterial(
        metallicFactor=0.0,
        alphaMode='OPAQUE',
        baseColorFactor=(1.0, 0.0, 0.0, 1.0))
    
    color_his = np.zeros((body_vertices_his.shape[1],3)).astype(float)
    color_his[:,0] = 1.0

    color_rec = np.zeros((body_vertices_his.shape[1],3)).astype(float)
    color_rec[:,2] = 1.0

    color_gt = np.zeros((body_vertices_his.shape[1],3)).astype(float)
    color_gt[:,1] = 1.0
    ...
    ## rendering
    if pkl_his is not None:
        for i in range(len(body_vertices_his)):
            body_rec = trimesh.Trimesh(body_vertices_his[i], body_faces_his, process=False)
            
            scene_m =trimesh.Scene()
            
            scene_m.add_geometry(
                trimesh.Trimesh(vertices=body_vertices_his[i], faces=body_faces_his, vertex_colors = color_his)
            )
            scene_m.add_geometry(scene_mesh)
            os.makedirs(save_folder, exist_ok = True)
            scene_m.export(os.path.join(save_folder, '{:0>3d}.ply'.format(i)))
            # scene_m.show()
            scene = pyrender.Scene()
            body_mesh_rec = pyrender.Mesh.from_trimesh(body_rec, material=material_his)
        ...
        for i in range(len(body_vertices_rec)):
            ...
            scene_m.add_geometry(
                trimesh.Trimesh(vertices=body_vertices_rec[i], faces=body_faces, vertex_colors = color_rec)
            )
            scene_m.add_geometry(scene_mesh)
            ...
            if body_vertices_gt is not None and i < len(body_vertices_gt):
                ...
                scene_m.add_geometry(
                trimesh.Trimesh(vertices=body_vertices_gt[i], faces=body_faces, vertex_colors = color_gt)
            )
            else:
                body_gt = None
            os.makedirs(save_folder, exist_ok = True)

            scene_m.export(os.path.join(save_folder, '{:0>3d}.ply'.format(i + len(body_vertices_his))))
            scene = pyrender.Scene()
            ...
            img = Image.fromarray((color * 255).astype(np.uint8))
            os.makedirs(save_folder, exist_ok = True)
            img.save(os.path.join(save_folder, '{:0>3d}.png'.format(i+len(body_vertices_his))))
