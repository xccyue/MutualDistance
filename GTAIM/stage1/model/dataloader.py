...
class DatasetGTA(Dataset):
    ...
    def _rotate_crop_sdf(self, location_origin,location, scene_data, sdf_len, R):
        ...
        # print('location',location,volume[50,50,50])
        ...
        # index1_ = torch.tensor([scene_data['xs_min'], scene_data['ys_min'], scene_data['zs_min']])
        # index2_ = torch.tensor([scene_data['xs_max'], scene_data['ys_max'], scene_data['zs_max']])
        # print('index error', index1 - index1_, index2-index2_)
        # index1 = index1.reshape(1,1,3)
        # index2 = index2.reshape(1,1,3)
    ...
    def _rotate_crop_sdf_v1(self, location_origin,location, scene_data, sdf_len, R):
        # radius = scene_data['radius']
        # voxel = 0.05
        scene_sdf = scene_data['sdf']
        ...
        index1 = torch.tensor([scene_data['xs_min'], scene_data['ys_min'], scene_data['zs_min']])
        index2 = torch.tensor([scene_data['xs_max'], scene_data['ys_max'], scene_data['zs_max']])
        # index1 = index1.reshape(1,1,3)
        # index2 = index2.reshape(1,1,3)

        volume = 2*((volume - index1)/(index2 - index1) - 0.5)
        # print(volume)
        scene_sdf1 = torch.tensor(scene_sdf).clone()
        scene_sdf1 = scene_sdf1.unsqueeze(0).float()
        volume = volume.float()
    ...
    def _get_origin_sdf_v1(self, location, scene_data, sdf_len):
        sdf = scene_data['sdf']
        sdf_len = 2*sdf_len
        location = location.reshape(3)
        bx, by, bz = sdf.shape[0], sdf.shape[1], sdf.shape[2]
        ...
        index1[1] = new_point[1]
        index1[2] = new_point[2]
        new_point1 = np.array([rightx,righty,rightz]).astype(np.float32)
