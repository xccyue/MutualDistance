...
class MotionGRU(nn.Module):
    def __init__(self, config):
        ...
        self.d_rnn = RNN(ny + nh_rnn + ns + nsdf+32 + nh_rnn, nh_rnn, cell_type=rnn_type)
        self.d_mlp = MLP(nh_rnn, nh_mlp)
        self.encode_beta = MLP(10, [10,32])
    ...
    def forward(self, x, fs, fsdf, beta, fhsdf):
        # fsdf S, B, sdf_out
        ...
        for i in range(self.horizon):
            ...
            # print(fsdfi.shape)
            rnn_in = torch.cat([h_x, y_p, fs, fsdfi,beta, fhsdfi], dim=1)
            h = self.d_rnn(rnn_in)
            h = self.d_mlp(h)
            output = self.d_out(h)
...
class GCN_H(nn.Module):
    def __init__(self, config, num_stage=12):
        ...
        input_feature = config.dct_n + 128 + 128 + 10
    ...
    def forward(self, sdf_in, fs,fh,beta):
        B, N, L = sdf_in.shape
        ...
        x = sdf_in
        x_in = torch.cat([sdf_in,fs,fh,beta], dim = -1)
